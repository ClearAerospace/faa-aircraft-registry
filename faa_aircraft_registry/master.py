...
def read(csvfile):
    ...
    for row in reader:
        ...
        record = {
            ...
            'last_action_date': convert_date(row.get('LAST ACTION DATE', None).strip()),
            'certificate_issue_date': convert_date(row.get('CERT ISSUE DATE', None).strip()),
            ...
            'airworthiness_date': convert_date(row.get('AIR WORTH DATE', None).strip()),
            'other_names': get_other_names(row),
            'expiration_date': convert_date(row.get('EXPIRATION DATE', None).strip()),
